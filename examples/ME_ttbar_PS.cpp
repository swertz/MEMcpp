...
double computeJacobian(vector<TLorentzVector> p){
	...
	double E1  = p.at(1).E();
	double p1x = p.at(1).Px();
	double p1y = p.at(1).Py();
	double p1z = p.at(1).Pz();

	double E2  = p.at(4).E();
	double p2x = p.at(4).Px();
	double p2y = p.at(4).Py();
	double p2z = p.at(4).Pz();

	double E3  = p.at(0).E();
	double p3x = p.at(0).Px();
	double p3y = p.at(0).Py();
	double p3z = p.at(0).Pz();

	double E4  = p.at(2).E();
	double p4x = p.at(2).Px();
	double p4y = p.at(2).Py();
	double p4z = p.at(2).Pz();

	double E5  = p.at(3).E();
	double p5x = p.at(3).Px();
	double p5y = p.at(3).Py();
	double p5z = p.at(3).Pz();
...
TFDISTR::TFDISTR(std::string paramCardPath){
...
double TFDISTR::Density(int nDim, double *Xarg){
  ...
  p3.SetPtEtaPhiM(50*tan(TMath::Pi()*Xarg[4]/2.), -log(TMath::Tan(TMath::Pi()*TMath::Pi()*Xarg[12]/2.)), 2*TMath::Pi()*Xarg[5], 0.);
  p4.SetPtEtaPhiM(50*tan(TMath::Pi()*Xarg[6]/2.), -log(TMath::Tan(TMath::Pi()*TMath::Pi()*Xarg[13]/2.)), 2*TMath::Pi()*Xarg[7], 0.);
  p5.SetPtEtaPhiM(50*tan(TMath::Pi()*Xarg[8]/2.), -log(TMath::Tan(TMath::Pi()*TMath::Pi()*Xarg[14]/2.)), 2*TMath::Pi()*Xarg[9], 0.);
  p6.SetPtEtaPhiM(50*tan(TMath::Pi()*Xarg[10]/2.), -log(TMath::Tan(TMath::Pi()*TMath::Pi()*Xarg[15]/2.)), 2*TMath::Pi()*Xarg[11], 0.);
  //Met.SetPtEtaPhiM(tan(TMath::Pi()*Xarg[12]/2.), 0., 2*TMath::Pi()*Xarg[13], 0.);

  /*p3.SetTheta(TMath::Pi()*Xarg[12]);
  p4.SetTheta(TMath::Pi()*Xarg[13]);
  p5.SetTheta(TMath::Pi()*Xarg[14]);
  p6.SetTheta(TMath::Pi()*Xarg[15]);*/
  //Met.SetTheta(TMath::Pi()*Xarg[18]);
  ...
  for(unsigned int i=0; i<E1.size(); i++){
	...
	double PhaseSpaceIn = 1.0 / ( 2. * TMath::Abs(q1Pz/(SQRT_S/2.)) * TMath::Abs(q2Pz/(SQRT_S/2.)) * pow(SQRT_S,2)); 
  ...
  integrand *= pow(50,4.) * pow(TMath::Pi(),4.) * pow(2*TMath::Pi(),4) * pow(TMath::Pi()/2.,4) / pow(TMath::Cos(TMath::Pi()*Xarg[4]/2.)*TMath::Cos(TMath::Pi()*Xarg[6]/2.)*TMath::Cos(TMath::Pi()*Xarg[8]/2.)*TMath::Cos(TMath::Pi()*Xarg[10]/2.),2.);
...
double ME(TLorentzVector ep, TLorentzVector mum, TLorentzVector b, TLorentzVector bbar, TLorentzVector Met, int nCells = 2000, int nSampl = 100, int nPoints = 50000){
  ...
  TFoamIntegrand *rho= new TFDISTR("/home/fynu/swertz/scratch/Madgraph/madgraph5/cpp_ttbar_epmum/Cards/param_card.dat");
...
//const char *inputFile,const char *outputFile
{
  ...
  for(Int_t entry = 0; entry < 1 ; ++entry)//numberOfEntries; ++entry)
  {
  ...
  weight1 = ME(gen_ep, gen_mum, gen_b, gen_bbar, gen_Met, 10000, 200, 100000);
  count_perm = 2;
  /*weight2 = ME(gen_ep, gen_mum, gen_bbar, gen_b, gen_Met, 2000, 100, 20000);
