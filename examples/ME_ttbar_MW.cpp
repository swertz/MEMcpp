...
public:
  ...
  //cout << "p3=" << p3.Pt() << ",p4=" << p4.Pt() << ",p5=" << p5.Pt() << ",p6=" << p6.Pt() << endl;
  }
  ...
  if(Xarg[0] == 1 || Xarg[1] == 1 || Xarg[2] == 1 || Xarg[3] == 1){
	cout << "Not 1!" << endl;
	mycount++;
	return 0.;
  }
...
double ME(TLorentzVector ep, TLorentzVector mum, TLorentzVector b, TLorentzVector bbar, TLorentzVector Met){
  ...
  FoamX->SetkDim(4);
  FoamX->SetnCells(40);      // No. of cells, can be omitted, default=2000
  FoamX->SetnSampl(10);
  ...
  for(Long_t loop=0; loop<400; loop++){
...
//const char *inputFile,const char *outputFile
{
  ...
  for(Int_t entry = 2; entry < 4 ; ++entry)//numberOfEntries; ++entry)
  {
